...
void
UnionFind::add(std::string elem) {
    this->elements[elem] = elem;
    this->lookup_leaves[elem] = 0;
}

std::string
UnionFind::root(std::string node, bool use_path_compression) {
    std::string n = node;
...
bool
UnionFind::find(std::string node_x, std::string node_y) {
    return this->root(node_x, false) == this->root(node_y, false);
}

void
UnionFind::unite(std::string node_x, std::string node_y, bool use_path_compression) {
