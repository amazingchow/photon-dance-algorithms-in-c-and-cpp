...
static lru_qnode_t *
new_lru_qnode(unsigned page_num) {
...
static lru_queue_t *
new_lru_queue(int frames_total) {
...
void
lru_put(lru_queue_t *queue, uint32_t page_num) {
...
// A function to add a page with given 'page_num' to LRU Queue.
void
lru_get(lru_queue_t *queue, uint32_t page_num) {
    lru_qnode_t *ref;
